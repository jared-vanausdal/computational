...
def ball_step(x,y,z,vx,vy,vz,g,ball,S0,w_vector,dt):
    ...
    vz = vz -(((ball.drag*v_vector.mag*vz)/ball.mass)*dt) + FM.z*dt #Changes vz based on the Magnus acceleration in the Z direction
...
def ball_calc(x,y,z,vx,vy,vz,g,m,B2,S0,w_vector,dt):
    ...
    ball.mass = m
    ball.drag = B2
    '''Initializes tee and golfball with attributes of drag and mass defined for practice with defining attributes of
    objects. I'm assuming it will come in handy later, and the practice now will be usefull.'''
...
vx = v0*math.cos(theta_rad) #finds components of velocity
...
w = (5000*(2*math.pi))/60#5822.78***
wz = w*math.cos((math.pi/4)) #finds the components of the spin and defines a vector using those components
